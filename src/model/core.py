...
class ApertisConfig:
    def __init__(
        ...
        attention_type: str = "standard_mha", # DEFAULT CHANGED
        ssm_d_inner: Optional[int] = None, 
        ssm_d_state: int = 16,       
        ssm_dt_rank: Union[int, str] = "auto", 
        ssm_conv_kernel: int = 4,    
        use_flash_attention: bool = False, # Note: FlashAttention is for quadratic attention
        ...
        self.use_flash_attention = use_flash_attention # Currently only relevant if attention_type is MHA
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, seq_len: Optional[int] = None):
        ...
        x_out[..., 0::2], x_out[..., 1::2] = rotated_x_part1, rotated_x_part2
...
class ApertisAttention(nn.Module):
    def __init__(self, config: ApertisConfig):
        ...
        if config.attention_type == "selective_ssm" or config.attention_type == "selective_linear": 
            if config.attention_type == "selective_linear" and config.attention_type != "selective_ssm": # only warn if different
    ...
    def forward(self, hidden_s: torch.Tensor, att_mask: Optional[torch.Tensor]=None, 
        ...
        if isinstance(self.attention_mechanism, SelectiveLinearAttention): 
            ...
            if att_mask is not None: 
                if att_mask.ndim == 4: key_padding_mask_bool = (att_mask.squeeze(1).squeeze(1) < -1e4) 
                elif att_mask.ndim == 2: key_padding_mask_bool = (att_mask == 0)
            att_out, att_weights_proxy = self.attention_mechanism(
                normed_hidden_s, normed_hidden_s, normed_hidden_s, 
                key_padding_mask=key_padding_mask_bool, need_weights=output_att
            )
            att_out = self.mha_o_proj(att_out)
        else: raise TypeError(f"Unknown attention mechanism type: {type(self.attention_mechanism)}")
...
class ApertisForCausalLM(nn.Module):
    ...
    def generate(
        ...
        initial_position_ids = kwargs.pop("position_ids", None) 
        if initial_position_ids is None:
            initial_position_ids = torch.arange(prompt_len, device=input_ids.device).unsqueeze(0).expand(batch_size, -1)
        for _ in range(max_new_tokens):
            current_model_input_ids = generated_tokens if internal_past_kv is None else generated_tokens[:, -1].unsqueeze(-1)
            pos_ids_for_prep = initial_position_ids if internal_past_kv is None else None 
            model_inputs = self.prepare_inputs_for_generation(
                input_ids=current_model_input_ids, 
                past_key_values=internal_past_kv,
                attention_mask=current_attention_mask, 
                position_ids=pos_ids_for_prep, 
...
def create_apertis_model(
    ...
    use_expert_system: bool = False,
    attention_type_override: Optional[str] = None, # Allow overriding attention type
    ssm_d_inner: Optional[int] = None,
    ssm_d_state: int = 16,
    ssm_dt_rank: Union[int, str] = "auto",
    ssm_conv_kernel: int = 4,
    ...
    # Set attention_type first if overridden, then ApertisConfig handles ssm_d_inner based on it
    if attention_type_override is not None:
        config_dict["attention_type"] = attention_type_override
    else: # Ensure default is set if not overridden
    ...
    config_dict.update({
        ...
        "ssm_d_inner": ssm_d_inner, # ApertisConfig will derive if None and attention_type is SSM
